...
from .common import WEB_HOST, common_request
...
async def request_user(id: str, cookie: str, offset: int = 0, limit: int = 20) -> dict:
    ...
    ret = {}
    tasks = [
        request_user_detail(id, cookie),
        request_user_notes(id, cookie, offset, limit),
    ]
...
async def request_user_detail(id: str, cookie: str) -> dict:
    ...
    if response.status_code != 200 or response.text == "":
        logger.error(
            f"failed get xhs user detail，id: {id}, code：{response.status_code}， body: {response.text}"
        )
        return {}
...
# 获取作品
async def request_user_notes(
    id: str, cookie: str, offset: int = 0, limit: int = 20
) -> list:
    ...
    while has_more and len(notes) < end_length:
        ...
        resp, succ = await common_request(
            "/api/sns/web/v1/user_posted", params, headers, True, False
        )
        if not succ:
            return []
